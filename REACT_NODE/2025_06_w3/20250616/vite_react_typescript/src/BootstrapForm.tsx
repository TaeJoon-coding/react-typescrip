...
function BootstrapForm() {
    ...
    const formNameCheck = (e : Event) => {
        let nameFld : HTMLInputElement
            = document.getElementById("menu_name") as HTMLInputElement;
        let nameRegex : RegExp = /^[一-龠ぁ-ゔァ-ヴー々0-9A-Za-z]{2,100}$/;
    ...
    const formEmailCheck = (e : Event) => {
        let emailFld : HTMLInputElement
            = document.getElementById("menu_email") as HTMLInputElement;
    ...
    const formPhoneCheck = (e : Event) => {
        let phoneFld : HTMLInputElement
        = document.getElementById("menu_phone") as HTMLInputElement;
        let phoneRegex : RegExp = /^\d{3}\d{4}\d{4}$/; //  ex) 01012345678
    ...
    /* 本文 */
    const formDescription =(e : Event) => {
        let descriptioFld : HTMLInputElement
        = document.getElementById("menu_description") as HTMLInputElement;
        // let descriptionRegex = /[가-힣]{2,}/; // 최소 2글자 이상
        // 韓国語の場合
        let descriptionRegex : RegExp = /^[一-龠ぁ-ゔァ-ヴー々〆〤0-9A-Za-z、。，．・？！（）［］｛｝「」『』【】《》〈〉“”（）‘’『』_~:;'"\-=+*\/&%$#@!?.,\s\(\)\[\]\^_\{\}\|]{3,}$/; 
        // 日本語の正規表現、最低3文字以上
    ...
    // AJAXフォルム転送(fetch)
    const formSubmit = async(e : HTMLInputElement) => {
        console.log("e target : ", e.target);
        
        // 因子
        let name : string = (document.getElementById("menu_name") as HTMLInputElement).value;
        let email : string = (document.getElementById("menu_email") as HTMLInputElement).value;
        let phone : string = (document.getElementById("menu_phone") as HTMLInputElement).value;
        let description : string = (document.getElementById("menu_description") as HTMLInputElement).value;
            ...
            <div className="member_wrap">
                ...
                <div className="btn-fld-pnl">
                    ...
                    <div className="btn_type_1">
                        {/* イベントハンドラー処理 */}
                        <Button onClick={(e : Event) => formSubmit(e)} className="btn_style_1">
                            提出
                        </Button>
                    </div>

                    <div className="btn_type_1">
                        <Button onClick={(e : Event) => formatInit(e)} className="btn_style_2">
                            キャンセル
                        </Button>
                    </div>
